...
import { fmt, monthlyAmount, MONTHS } from "@/lib/finance";
import { Header } from "@/components/app/Header";
import { Bar, BarChart, CartesianGrid, Cell, Pie, PieChart, ResponsiveContainer, Tooltip, XAxis, YAxis, Legend } from "recharts";

export default function Anual() {
  const { fixedItems, transactions } = useFinance();
  ...
  const byCategory = useMemo(() => {
    ...
    fixedItems.filter((i) => i.type === "expense_fixed" || i.type === "expense_variable").forEach((i) => {
      map[i.category] = (map[i.category] || 0) + monthlyAmount(i) * 12;
    });
  ...
  return (
    <div>
      <Header title="Resumen anual" subtitle={`${new Date().getFullYear()} en una vista`} />
