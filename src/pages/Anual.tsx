...
interface MonthRow {
  ...
  Ingresos: number; Gastos: number; Ahorros: number; Neto: number; Tasa: number;
}

const COLORS = [
  "hsl(var(--primary))", "hsl(var(--secondary))", "hsl(var(--accent))",
  "hsl(var(--success))", "hsl(var(--warning))", "hsl(var(--destructive))",
  "hsl(var(--primary-glow))", "hsl(250 95% 65%)",
];
...
export default function Anual() {
  ...
  const buildMonthly = useMemo(() => {
    return (year: number): MonthRow[] => MONTHS.map((_, idx) => {
      ...
      return { mes: MONTHS_SHORT[idx], idx, Ingresos: Math.round(Math.max(0, income)), Gastos: Math.round(Math.max(0, expense)), Ahorros: Math.round(Math.max(0, saving)), Neto: Math.round(income - expense - saving), Tasa: Math.round(tasa * 10) / 10 };
    });
  ...
  const totals = useMemo(() => monthly.reduce((a, b) => ({ income: a.income + b.Ingresos, expense: a.expense + b.Gastos, saving: a.saving + b.Ahorros, net: a.net + b.Neto }), { income: 0, expense: 0, saving: 0, net: 0 }), [monthly]);
  const totalsPrev = useMemo(() => monthlyPrev.reduce((a, b) => ({ income: a.income + b.Ingresos, expense: a.expense + b.Gastos, saving: a.saving + b.Ahorros, net: a.net + b.Neto }), { income: 0, expense: 0, saving: 0, net: 0 }), [monthlyPrev]);
  const avg = { income: totals.income / 12, expense: totals.expense / 12, saving: totals.saving / 12, net: totals.net / 12 };
  ...
  const byCategory = useMemo(() => {
    ...
    transactions.filter((t) => t.type === "expense" && parseDateLocal(t.date).getFullYear() === activeYear).forEach((t) => {
      if (t.type === "transfer" || (t.transferToAccountId && accounts.some((a) => a.id === t.transferToAccountId))) return;
  ...
  const topConcepts = useMemo(() => {
    ...
    return Object.entries(map).map(([name, v]) => ({ name, ...v })).sort((a, b) => b.amount - a.amount).slice(0, 6);
  }, [transactions, activeYear, accounts]);

  const byMethod = useMemo(() => {
    const map: Record<PaymentMethod, number> = { cash: 0, transfer: 0, card: 0, other: 0 };
    transactions.filter((t) => t.type === "expense" && parseDateLocal(t.date).getFullYear() === activeYear).forEach((t) => {
      if (t.paymentMethod && !(t.type === "transfer" || (t.transferToAccountId && accounts.some((a) => a.id === t.transferToAccountId)))) {
    ...
    return (Object.keys(map) as PaymentMethod[]).map((k) => ({ key: k, name: PAYMENT_METHOD_LABEL[k], emoji: PAYMENT_METHOD_EMOJI[k], value: Math.round(map[k]) })).filter((x) => x.value > 0).sort((a, b) => b.value - a.value);
  ...
  const debtsYear = useMemo(() => {
    const lent = debts.filter((d) => parseDateLocal(d.date).getFullYear() === activeYear).reduce((s, d) => s + d.amount, 0);
    const collected = debts.flatMap((d) => d.payments).filter((p) => parseDateLocal(p.date).getFullYear() === activeYear).reduce((s, p) => s + p.amount, 0);
    const outstanding = debts.reduce((s, d) => { const paid = d.payments.reduce((a, p) => a + p.amount, 0); return s + Math.max(0, d.amount - paid); }, 0);
  ...
  return (
    <div>
      <Header title="Resumen anual" subtitle="Tu año en una sola vista"
        action={
          <button onClick={exportCsv} className="size-10 rounded-xl bg-card border border-border shadow-soft flex items-center justify-center hover:bg-muted transition" aria-label="Exportar CSV">
            <Download className="size-4" />
          </button>
        } />

      <div className="px-5 mb-4 flex justify-center">
        <div className="inline-flex items-center gap-1 rounded-xl bg-card border border-border px-1.5 py-1.5 shadow-soft">
          <button onClick={() => setActive(activeYear - 1, activeMonth)} className="size-8 rounded-xl hover:bg-muted flex items-center justify-center transition" aria-label="Año anterior"><ChevronLeft className="size-4" /></button>
          <span className="px-4 h-8 flex items-center text-base font-extrabold tracking-tight">{activeYear}</span>
          <button onClick={() => setActive(activeYear + 1, activeMonth)} className="size-8 rounded-xl hover:bg-muted flex items-center justify-center transition" aria-label="Año siguiente"><ChevronRight className="size-4" /></button>
        </div>
      </div>

      <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        className="mx-5 rounded-2xl gradient-primary text-primary-foreground p-5 shadow-glow relative overflow-hidden">
      ...
      <section className="px-5 mt-4">
        <div className="rounded-xl bg-card border border-border p-4 shadow-soft">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs uppercase tracking-wider font-semibold text-muted-foreground">Tasa de ahorro anual</span>
            <span className="text-sm font-bold text-success">{savingsRate.toFixed(1)}%</span>
          </div>
          <div className="h-2 rounded-full bg-muted overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(100, savingsRate)}%` }} transition={{ duration: 1, ease: "easeOut" }} className="h-full gradient-success rounded-full" />
          </div>
          <p className="text-xs text-muted-foreground mt-2">Promedio mensual: {fmt(avg.income)} / {fmt(avg.expense)} / {fmt(avg.saving)}</p>
        </div>
      </section>

      <div className="px-5 mt-5">
        <PillTabs<Tab> ariaLabel="Vistas del resumen anual" tabs={["general", "categorias", "metodos", "metas"]} value={tab} onChange={setTab} />
      </div>
        ...
        <>
          {(bestNet || worstNet) && (
            <section className="px-5 mt-5 grid grid-cols-2 gap-3">
              {bestNet && <HighlightCard tone="success" icon={<Trophy className="size-4" />} label="Mejor mes" primary={MONTHS[bestNet.idx]} secondary={`Neto ${fmt(bestNet.Neto)}`} onClick={() => { setActive(activeYear, bestNet.idx); navigate("/"); }} />}
              {worstNet && worstNet.Neto < 0 && <HighlightCard tone="danger" icon={<AlertTriangle className="size-4" />} label="Peor mes" primary={MONTHS[worstNet.idx]} secondary={`Neto ${fmt(worstNet.Neto)}`} onClick={() => { setActive(activeYear, worstNet.idx); navigate("/"); }} />}
              {topExpenseMonth && (!worstNet || worstNet.Neto >= 0) && <HighlightCard tone="warning" icon={<TrendingDown className="size-4" />} label="Más gasto" primary={MONTHS[topExpenseMonth.idx]} secondary={fmt(topExpenseMonth.Gastos)} onClick={() => { setActive(activeYear, topExpenseMonth.idx); navigate("/"); }} />}
            </section>
          )}
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-5 lg:px-5">
            <section className="px-5 lg:px-0 mt-5">
              ...
              <ChartCard>
                <SimpleAreaChart data={monthly} xKey="mes" height={240} series={[
            ...
            <section className="px-5 lg:px-0 mt-5">
              ...
              <ChartCard>
                <SimpleAreaChart data={monthly} xKey="mes" height={220} series={[
          ...
          {(debtsYear.lent + debtsYear.collected + debtsYear.outstanding > 0) && (
            <section className="px-5 mt-5">
              ...
              <div className="rounded-xl bg-card border border-border p-4 shadow-soft grid grid-cols-3 gap-3">
      ...
      {tab === "categorias" && (
        <div className="lg:grid lg:grid-cols-2 lg:gap-5 lg:px-5">
          <section className="px-5 lg:px-0 mt-5">
            ...
            <ChartCard>
              {byCategory.length === 0 ? <EmptyState text={`Sin gastos registrados en ${activeYear}`} /> : (
          ...
          <section className="px-5 lg:px-0 mt-5">
            ...
            {topConcepts.length === 0 ? <ChartCard><EmptyState text="Aún no hay movimientos variables" /></ChartCard> : (
              <div className="space-y-1.5">
                {topConcepts.map((c) => (
                  <div key={c.name} className="rounded-xl bg-card border border-border p-3 shadow-soft flex items-center gap-3">
                    <IconDisplay icon={iconFor(c.sample)} />
                    <div className="flex-1 min-w-0"><p className="font-semibold text-sm truncate">{c.name}</p><p className="text-xs text-muted-foreground">{c.count} vez{c.count === 1 ? "" : "es"}</p></div>
      ...
      {tab === "metodos" && (
        <section className="px-5 mt-5">
          ...
          <ChartCard>
            {byMethod.length === 0 ? <EmptyState text="Sin métodos de pago registrados" /> : (
              <div className="grid grid-cols-2 gap-2 mt-1">
                {byMethod.map((m) => {
                  const total = byMethod.reduce((s, x) => s + x.value, 0);
                  const pct = total > 0 ? (m.value / total) * 100 : 0;
                  return (
                    <div key={m.key} className="rounded-xl bg-muted/60 p-3">
                      <p className="text-xs font-semibold flex items-center gap-1.5"><span className="text-base">{m.emoji}</span>{m.name}</p>
          ...
          {goalsThisYear.length === 0 ? (
            <ChartCard>
              <div className="text-center py-6">
                ...
                <button onClick={() => navigate("/metas")} className="mt-3 text-xs font-bold text-primary hover:underline">Ir a Metas →</button>
              </div>
            </ChartCard>
          ) : (
              ...
              {goalsThisYear.map(({ goal, contribsYear }) => {
                ...
                return (
                  <button key={goal.id} onClick={() => navigate("/metas")}
                    className={`block w-full text-left rounded-xl p-4 text-primary-foreground shadow-pop relative overflow-hidden ${goal.color}`}>
                    ...
                    <div className="relative flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs opacity-80">{fmt(goal.saved)} / {fmt(goal.target)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xl font-extrabold">{Math.round(pct)}%</p>
                        <p className="text-[10px] opacity-70 uppercase tracking-wider">avance</p>
                      </div>
                    </div>
                    ...
                    <p className="text-xs opacity-80 mt-2"><Sparkles className="inline size-3 mr-1" />Aportado en {activeYear}: <span className="font-bold">{fmt(contribsYear)}</span></p>
                  </button>
                );
              })}
            </div>
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h2 className="text-sm font-extrabold tracking-tight mb-3">{children}</h2>;
}
function ChartCard({ children }: { children: React.ReactNode }) {
  return <div className="rounded-xl bg-card border border-border p-4 shadow-soft">{children}</div>;
}
function EmptyState({ text }: { text: string }) {
  return <p className="text-sm text-muted-foreground text-center py-8">{text}</p>;
}
function MiniStat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="rounded-xl bg-white/15 p-2.5 backdrop-blur-sm"><div className="flex items-center gap-1 opacity-90 mb-0.5">{icon}<span className="text-[10px] uppercase tracking-wider font-semibold">{label}</span></div><p className="text-sm font-bold truncate">{value}</p></div>;
}
function YoyBadge({ current, previous }: { current: number; previous: number }) {
  ...
  return <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-white/20 px-2.5 py-1 text-xs font-bold">{positive ? <TrendingUp className="size-3" /> : <TrendingDown className="size-3" />}{positive ? "+" : ""}{pct.toFixed(0)}% vs año anterior</div>;
}
function YoyCard({ label, current, previous, positive }: { label: string; current: number; previous: number; positive: boolean }) {
  ...
  return <div className="rounded-xl bg-card border border-border p-3 shadow-soft"><p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">{label}</p><p className="text-base font-extrabold mt-1">{fmt(current)}</p><p className={`text-xs font-bold mt-0.5 ${tone}`}>{diff >= 0 ? "▲" : "▼"} {Math.abs(pct).toFixed(0)}% <span className="text-muted-foreground font-normal">· {fmt2(previous)}</span></p></div>;
}
function HighlightCard({ tone, icon, label, primary, secondary, onClick }: { tone: "success" | "danger" | "warning"; icon: React.ReactNode; label: string; primary: string; secondary: string; onClick?: () => void }) {
  const map = { success: "bg-success/10 text-success", danger: "bg-destructive/10 text-destructive", warning: "bg-warning/10 text-warning" };
  return <button onClick={onClick} className="rounded-xl bg-card border border-border p-3 shadow-soft text-left active:scale-[0.98] transition"><div className={`size-8 rounded-xl flex items-center justify-center ${map[tone]}`}>{icon}</div><p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold mt-2">{label}</p><p className="text-sm font-extrabold mt-0.5">{primary}</p><p className="text-xs text-muted-foreground">{secondary}</p></button>;
}
function DebtStat({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: string; tone: "success" | "danger" | "warning" }) {
  const map = { success: "bg-success/10 text-success", danger: "bg-destructive/10 text-destructive", warning: "bg-warning/10 text-warning" };
  return <div className="text-center"><div className={`size-9 mx-auto rounded-xl flex items-center justify-center ${map[tone]}`}>{icon}</div><p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold mt-2">{label}</p><p className="text-sm font-extrabold mt-0.5">{value}</p></div>;
}
